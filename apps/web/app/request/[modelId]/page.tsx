...
function RequestFormContent() {
  ...
  const { models, createBookingRequest } = useMockState();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      const successParams = new URLSearchParams({
        ref: newBooking.publicReference,
        token: newBooking.privateStatusToken,
        model: model.name,
        brand: model.brand,
      });
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="mb-6">
        <Link
          href={`/models/${model.id}?pickupDate=${pickupDate}&pickupTime=${pickupTime}&returnDate=${returnDate}&returnTime=${returnTime}`}
