...
export default function OwnerCalendarPage() {
  const { bookings, vehicles, models, blocks, closures } = useMockState();
  const [viewMode, setViewMode] = useState<'week' | 'day'>('week');
  const [selectedDate, setSelectedDate] = useState<string>('2026-09-06');

  // Days in week: 2026-09-05 through 2026-09-11
  const weekDays = [
    '2026-09-05',
    '2026-09-06',
    '2026-09-07',
  ...
  const getEventsForDate = (dateStr: string) => {
    ...
    const dayBookings = bookings.filter(b => {
      const p = new Date(b.confirmedPickupAt || b.requestedPickupAt);
      const r = new Date(b.confirmedReturnAt || b.requestedReturnAt);
              ...
              {weekDays.map(dateStr => {
                ...
                return (
                  <div
                    ...
                    className={`bg-surface border rounded-card p-3.5 flex flex-col justify-between cursor-pointer transition-all min-h-[220px] ${
                      isSelected
                    ...
                    <div>
                      <div className="flex items-center justify-between pb-2 mb-2 border-b border-border/60">
                        <div>
                          ...
                          <div className="text-base font-extrabold text-text">{dayNum} Sep</div>
                        </div>
                      ...
                      <div className="space-y-1.5">
                        {cls.map(c => (
                          <div
                            key={c.id}
                            className="p-1.5 rounded bg-amber-100 text-amber-900 border border-amber-200 text-[10px] font-bold truncate"
                          >
                        ...
                        {blks.map(b => {
                          ...
                          return (
                            <div
                              key={b.id}
                              className="p-1.5 rounded bg-rose-100 text-rose-900 border border-rose-200 text-[10px] font-bold truncate"
                            >
                              ⛔ {veh?.internalCode}: Blocked
                            </div>
                          );
                        })}
                        {bks.map(bk => {
                          const model = models.find(m => m.id === bk.modelId);
                          const veh = vehicles.find(v => v.id === bk.assignedVehicleId);
                          return (
                            <div
                              key={bk.id}
                            ...
                            >
                              🚗 {veh ? veh.internalCode : model?.name}: {bk.customerName.split(' ')[0]}
                            </div>
                          );
                        })}
                      </div>
                ...
                {selectedDayEvents.blocks.map(b => {
                  ...
                  return (
                    <div key={b.id} className="p-3 bg-rose-50 border border-rose-200 rounded-control flex items-center justify-between text-xs">
                      <div>
                        <span className="font-bold text-rose-950">⛔ Vehicle Block: {veh?.internalCode}</span>
                    ...
                    <div key={bk.id} className="p-4 bg-surface rounded-card border border-border shadow-sm flex items-center justify-between text-xs hover:bg-surface-alt/40 transition-colors">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-brand">{model?.brand} {model?.name}</span>
                          {vehicle && (
                        ...
                        <p className="text-[11px] text-text-muted">
                          Pickup: {formatDateTime(bk.confirmedPickupAt || bk.requestedPickupAt)} | Return: {formatDateTime(bk.confirmedReturnAt || bk.requestedReturnAt)}
                        </p>
                      </div>
